...
import com.fetters.picture.model.dto.space.analyze.SpaceAnalyzeRequest;
...
/**
 * @author : Fetters
 * @description : 空间图片分析服务实现类
 * @createDate : 2025/10/12 19:33
 */
public class SpaceAnalyzeServiceImpl implements SpaceAnalyzeService {
    ...
    public SpaceUsageAnalyzeResponse getSpaceUsageAnalyze(SpaceUsageAnalyzeRequest spaceUsageAnalyzeRequest, User loginUser) {
        ...
        if (spaceUsageAnalyzeRequest.isQueryAll() || spaceUsageAnalyzeRequest.isQueryPublic()) {
            ...
            ThrowUtils.throwIf(!isAdmin, ErrorCode.NO_AUTH_ERROR, "无权访问空间图片");
            // 统计公共图库的资源使用
            ...
            ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间图片不存在");

            // 权限校验：仅空间图片所有者或管理员可访问
     ...
     * @param loginUser                   当前登录用户
     * @return List<SpaceCategoryAnalyzeResponse> 空间图片分类分析结果响应列表
     */
    @Override
    ...
    /**
     * 校验空间图片分析权限
     * @param spaceAnalyzeRequest SpaceAnalyzeRequest 空间图片分析请求
     * @param loginUser           当前登录用户
     */
    private void checkSpaceAnalyzeAuth(SpaceAnalyzeRequest spaceAnalyzeRequest, User loginUser) {
        // 检查权限
        if (spaceAnalyzeRequest.isQueryAll() || spaceAnalyzeRequest.isQueryPublic()) {
            // 全空间图片分析或者公共图库权限校验：仅管理员可访问
     ...
     * 根据分析范围补充查询条件
     * @param spaceAnalyzeRequest SpaceAnalyzeRequest 空间图片分析请求
     * @param queryWrapper        QueryWrapper<Picture> 查询条件
     */
